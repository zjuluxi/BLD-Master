...
namespace Cube
{
    public class Cube4
    {
        ...
        private static readonly int[] WingIndex = new int[] { 14, 34, 33, 13, 8, 66, 65, 4, 1, 82, 81, 2,
            7, 18, 17, 11, 49, 45, 46, 50, 56, 77, 78, 52,
            62, 93, 94, 61, 55, 29, 30, 59, 39, 20, 24, 43,
            40, 75, 71, 36, 87, 68, 72, 91, 88, 27, 23, 84 },
                CornorIndex = new int[] { 12, 32, 67, 0, 64, 83, 3, 80, 19, 15, 16, 35,
        ...
        public static Cube4 RandomCube4(bool corner=true, bool wing=true, bool xcenter=true)
        {
            ...
            for (int i = 0; i < 8; i++) cp[i] = cp[i] * 3 + co[i];
            int[] w, xc;
            if (wing) w = Wing.Random();
            else { w = new int[24]; for (int i = 0; i < 24; i++) w[i] = i; }
            if (xcenter) xc = XCenter.Random();
            else { xc = new int[24]; for (int i = 0; i < 24; i++) xc[i] = i >> 2; }
        ...
        public void Copy(Cube4 cube)
        {
            corner.Copy(cube.corner);
            wing.Copy(cube.wing);
            xcenter.Copy(cube.xcenter);
        }
        public void Solve()
        {
            corner.Solve(0);
